 ...
 */
#ifndef FREEVARS_H_15224
#define FREEVARS_H_15224
#include<vector>
...
class FreshVars {
    public:
        ...
        bool unmark(SATSPC::Var v) {
            if (!is_marked(v)) return false;
            const auto i = ix(v);
            marked[i] = false;
            if (ix(fresh) > i) fresh = v;
            return true;
        }

        bool mark(SATSPC::Var v) {
            const auto i = ix(v);
            if (i >= marked.size()) marked.resize(i+1, false);
            const bool retv = !(marked[i]);
            marked[i] = true;
            while (is_marked(fresh)) ++fresh;
            return retv;
        }
    ...
    protected:
        ...
        inline size_t ix(SATSPC::Var v) const { assert(v >= 0); return (size_t) v; }
};

std::ostream& operator << (std::ostream& o, const FreshVars& f);
#endif /* FREEVARS_H_15224 */
