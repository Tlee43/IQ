 ...
 */
#ifndef ENCODER_H_25290
#define ENCODER_H_25290
#include"aig2cnf.h"
#include"LitSet.h"
#include"qtypes.h"
...
class Encoder {
public:
  Encoder(VarManager& vm, Level ql, AigFactory& factory, Minisat::MiniSatExt& sat_solver)
    : var_mng(vm)
    , aig_encoder(vm, ql, factory,sat_solver)
    , sat_solver(sat_solver)
    {}
  inline void alloc_var(Var _max_id) { aig_encoder.alloc_vars(_max_id); }
  inline Var fresh() { return aig_encoder.fresh(); }
  inline Var nextFree() { return aig_encoder.nextFree(); }
