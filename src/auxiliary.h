...
class MaybeIndex {
  public:
    bool operator() () const {return just;}
    size_t operator* () const {return index;}
    void operator=(size_t i) {
      index=i;
      just=true;
    }
    MaybeIndex(const MaybeIndex& o) : index(o.index), just(o.just) {}
    MaybeIndex(size_t i) : index(i), just(true) {}
    MaybeIndex() : index(-1), just(false) {}
  private:
    size_t index;
    bool   just;
};
...
template <class V>
std::vector<V>* mk_vec(const V& val) {
...
inline void insert_chk(std::unordered_set<V,Hash,Pred,Alloc>& m,
...
template <class V>
bool mark(std::unordered_set<V>& es, const V& e) {
    const auto i = es.find(e);
    if (i != es.end()) return false;
    es.insert(i, e);
    return true;
}
...
template <class T>
bool rm(std::vector<T>& vs,const T e) {
...
template <class T>
inline void cleanup(std::vector<T*>& vs) {
    auto i = vs.size();
    while(i) { i--; delete vs[i]; }
    vs.clear();
}

inline bool ends_with(const std::string& filename, const char* suf) {
    int l = strlen(suf);
    size_t s = filename.length();
    if ((size_t)l > s) return false;
    while (l)
        if (filename[--s] != suf[--l]) return false;
    return true;
}

template <class T>
inline void pop_chk(std::vector<T>& stack, const T& fsl) {
