...
size_t AigFactory::mk_node(AigLit& a, AigLit& b) {
    ...
    if(i!=nodes_map.end()) return i->second;
...
std::ostream& AigFactory::print(std::ostream& out, AigLit l, size_t off) {
    for (size_t i=0; i<off; ++i) out << ' ';
    if (l.is_false()) return out << "<F>";
    if (l.is_true()) return out << "<T>";
    if (l.is_var()) { return out << (l.sign() ? '-' : '+') << l.var(); }
    if (l.is_node()) {
        out << (l.sign() ? "-" : "+") << "AND";
        const auto n = get_node(l.ptr());
        vector<AigLit> ops;
        ...
        for (AigLit op : ops) if (op.is_var()) print(out << " ", op, 0);
        for (AigLit op : ops) if (!op.is_var()) print(out << endl, op, off+2);
        return out;
    }
    assert(0);
    return out;
...
std::ostream& AigFactory::print(std::ostream& out, AigLit l, size_t off, bool labels, bool pol) {
    for (size_t i=0; i<off; ++i) out << ' ';
    if (l.is_const()) return out << (pol==l.is_true() ? "<T>" : "<F>");
    const bool npol = pol == !l.sign();
    if (l.is_var()) { return out << (npol ? '+' : '-') << l.var(); }
    if (l.is_node()) {
        const auto n = get_node(l.ptr());
        ...
        if (labels) {
            ...
            for(AigLit& fl : flattened) out << ',' << fl.ptr();
            out << ']';
        }
        for (AigLit op : ops) if (op.is_var()) print(out << " ", op, 0, labels, npol);
        for (AigLit op : ops) if (!op.is_var()) print(out << endl, op, off+2, labels, npol);
        return out;
    }
    assert(0);
    return out;
