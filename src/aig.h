...
class AigLit {
    public:
        ...
        AigLit() {}
        inline static AigLit mk_node(size_t ptr, bool sign) { return AigLit(NODE, sign, ptr); }
        inline static AigLit mk_var(int v, bool sign) { assert(v>=0); return AigLit(VAR, sign, (size_t)v); }
        lit_type type() const {return _type;}
        int var() const { assert(type() == VAR); return data; }
        size_t ptr() const { assert(type() == NODE); return data; };
        size_t sign() const { assert(type() == NODE || type() == VAR); return _sign; }
        ...
        inline bool equals(const AigLit& o) const { return _type == o._type && data == o.data && _sign == o._sign; }
        ...
        inline static bool is_neg(const AigLit& a, const AigLit& b) {
            ...
            if (a.is_var()) return a.var() == b.var() && a.sign() != b.sign();
            if (a.is_node()) return a.ptr() == b.ptr() && a.sign() != b.sign();
            if (a.is_true()) return b.is_false();
            if (a.is_false()) return b.is_true();
            assert(0);
            return false;
        }
    ...
    private:
        ...
        bool     _sign;
        size_t   data;
        AigLit(lit_type t, bool s, size_t data)
                : _type(t)
                , _sign(s)
                , data(data) {}
...
inline bool operator < (const AigLit& n1,const AigLit& n2) { return AigLit::cmp(n1,n2) < 0; }

class AigNode {
    public:
        AigNode() {}
        static inline AigNode mk(AigLit& s, AigLit& t) { return s < t ? AigNode(s, t) : AigNode(t, s); }

        inline const AigLit& a() const {return _a;}
        inline const AigLit& b() const {return _b;}
        inline size_t hash() const { return a().hash() ^ b().hash(); }
        inline bool equals(const AigNode& o) const { return _a.equals(o._a) && _b.equals(o._b); }
    ...
    private:
        AigLit _a, _b;
        inline AigNode(AigLit& s, AigLit& t) : _a(s), _b(t) {}
};

struct AigNode_hash { inline size_t operator () (const AigNode& n) const { return n.hash(); } };
struct AigNode_equal { inline bool operator () (const AigNode& n1,const AigNode& n2) const { return n1.equals(n2); } };
...
namespace std {
    ...
    template<>
        class equal_to<AigLit> {
            public:
                inline bool operator () (const AigLit& n1,const AigLit& n2) const { return n1.equals(n2); }
        };

    template<>
        class less<AigLit> {
            public:
                inline bool operator () (const AigLit& n1,const AigLit& n2) const { return n1<n2; }
        };

    template<> class less<AigNode> { public: inline bool operator () (const AigNode& n1,const AigNode& n2) const { return n1.less(n2); } };
...
class AigFactory {
    ...
    public:
        ...
        inline AigLit mk_and(AigLit a, AigLit b, AigLit c) { return mk_and(a, mk_and(b,c)); }
        ...
        std::ostream& print_fancy(std::ostream& o, AigLit l, size_t off=0) { return print(o, l, off, false, true); }
        std::ostream& print(std::ostream& o, AigLit l, size_t off, bool labels, bool pol);
        AigNode get_node(size_t ptr) { return get_node_(ptr); }
        AigNode get_node(const AigLit& l) { assert(l.is_node()); return get_node_(l.ptr()); }
        size_t node_count() const {  return nodes.size(); }
    protected:
        //std::unordered_map<AigNode,size_t,AigNode_hash,AigNode_equal> nodes_map;
		// these list are used for has consing
        std::map<AigNode,size_t> nodes_map;
        std::vector<AigNode> nodes;
        size_t mk_node(AigLit& a, AigLit& b);
        AigNode& get_node_(size_t ptr) { assert(ptr<nodes.size()); return nodes[ptr]; }
...
// # define CHK_SAME(aa, ab, ba, bb)/    do {if ((aa).equals(ba)) { found_same = true; same = aa; aother = ab; bother = bb; }} while (0)

inline bool chk_same(const AigLit aa, const AigLit ab, const AigLit ba, const AigLit bb, AigLit& same, AigLit& aother, AigLit& bother) {
    if (!(aa).equals(ba)) return false;
    same = aa; aother = ab; bother = bb;
    return true;
}


//inline bool chk_neq(const AigLit aa, const AigLit ab, const AigLit ba, const AigLit bb, AigLit& same, AigLit& aother, AigLit& bother) {
...
bool AigFactory::simp_unit(AigLit a, AigLit conj, AigLit& res) {
   if (a.is_node() && conj.is_var()) std::swap(a,conj);
   else if (!a.is_var() || !conj.is_node()) return false;
   ...
   if (a.equals(n.a())) { res = conj.sign() ? mk_and(a, neg(n.b())) : conj; return true; }
   if (a.equals(n.b())) { res = conj.sign() ? mk_and(a, neg(n.a())) : conj; return true; }
   return false;
}

bool AigFactory::simp_nodes(AigLit a, AigLit b, AigLit& res) {
    if (!a.is_node() || !b.is_node()) return false;
    if (a.sign() && !b.sign()) std::swap(a,b);
...
#endif
        if (asg)  { assert(bsg); res = neg(mk_and(same, mk_or(aother, bother))); }
        else if (bsg)  { assert(!asg); res = mk_and(neg(bother), a); }
        else { assert(!asg && !bsg); res = mk_and(same, aother, bother); }
        return true;
    }
    if ((!asg || !bsg)
        && (AigLit::is_neg(na.a(), nb.a()) || AigLit::is_neg(na.a(), nb.b()) || AigLit::is_neg(na.b(), nb.a()) || AigLit::is_neg(na.b(), nb.b()))) {
        if (bsg)  { assert(!asg); res = a; }
        else { assert(!asg && !bsg); res = mk_false(); }
        return true;
    }
    return false;
}
...
AigLit AigFactory::_mk_and(AigLit a, AigLit b) {
    ...
    if (is_false(a) || is_false(b)) return mk_false();
    if (a.equals(b)) return a;
    if (AigLit::is_neg(a,b)) return mk_false();
#if 1
    AigLit tmp;
    if (simp_nodes(a, b, tmp)) return tmp;
    if (simp_unit(a, b, tmp)) return tmp;
#endif
