 ...
 */
#ifndef SUBSTITUTE_H_9111
#define SUBSTITUTE_H_9111
#include"aig.h"
...
class SubstituteAndFreshen : public Substitute {
    public:
        SubstituteAndFreshen(AigFactory& factory, const SATSPC::vec<lbool>& s, FreshVars& fvars)
            : Substitute(factory, s)
            , FRESHEN(-1)
            , fvars(fvars)
            {}
        ...
        inline void mark(Var v) {
            ...
            if (v >= sz) {
                ...
                for (Var i = sz; i < v; ++i) var_replace[i] = i;
            }
    ...
    protected:
        ...
        FreshVars& fvars;
        virtual bool subs_node(const AigLit& t, AigLit& res);
        inline Var mapped(Var v) const { return v >= var_replace.size() ? v : var_replace[v]; }
...
class SubstituteAndFreshenMap : public SubstituteBase {
    public:
        SubstituteAndFreshenMap(AigFactory& factory,
                const std::unordered_map<Var,AigLit>& s, FreshVars& fvars)
            : SubstituteBase(factory)
            , FRESHEN(-1)
            , s(s)
            , fvars(fvars)
