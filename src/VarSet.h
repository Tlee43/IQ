...
class const_ord_VarIterator : public std::iterator<std::forward_iterator_tag, Var>
{
public:
  inline const_ord_VarIterator(const VarSet& ls, size_t x);
  inline const_ord_VarIterator(const const_ord_VarIterator& mit) : ls(mit.ls), i(mit.i) {}
  inline const_ord_VarIterator& operator++();
  inline const Var operator*() const;
  bool operator==(const const_ord_VarIterator& rhs) { assert(&ls==&(rhs.ls)); return i==rhs.i;}
  bool operator!=(const const_ord_VarIterator& rhs) { assert(&ls==&(rhs.ls)); return i!=rhs.i;}
private:
  const VarSet&               ls;
  size_t                      i;
...
private:
  ...
  std::vector<Var>         vs;
  VarSet(const VarSet& o) {assert(0);}
};
...
inline bool VarSet::remove(Var v) {
  ...
  if (s.size() <= vix || !s[vix]) return false;
  s[vix] = false;
...
inline const_ord_VarIterator::const_ord_VarIterator(const VarSet& ls, size_t x) : ls(ls), i(x) {
  while ((i<ls.physical_size()) && !ls.get(i)) ++i;
}
inline const Var const_ord_VarIterator::operator*() const {
  assert(ls.get(i));
  return (Var)i;
}
...
inline const_ord_VarIterator VarSet::ord_end()   const { return const_ord_VarIterator(*this, physical_size()); }
inline const_ord_VarIterator VarSet::ord_begin() const { return const_ord_VarIterator(*this, 0); }
