 ...
 */
#ifndef AIG_UTIL_H_32320
#define AIG_UTIL_H_32320
#include"aig.h"
#include"qtypes.h"
#include"Core.h"
class AigUtil {
    public:
        ...
        inline AigLit convert(const Lit& l) { return factory.mk_var(var(l), sign(l)); }
    ...
    private:
        ...
        inline bool get_eval(AigLit t, const Core& assignment, std::unordered_map<size_t,lbool>& node_cache, lbool& rese);
        inline bool aux_sz(const AigLit& t, std::unordered_map<size_t,size_t>& node_cache, size_t& res);
        inline bool aux_depth(const AigLit& t, size_t& res);
};

bool AigUtil::get_eval(AigLit t, const Core& assignment, std::unordered_map<size_t,lbool>& node_cache, lbool& res) {
    if (t.is_node()) {
        ...
        res = (vl == SATSPC::lit_Undef) ? SATSPC::l_Undef
                                : ((t.sign() == sign(vl)) ? l_True : l_False);
    }
    else if (t.is_true()) res = l_True;
    else if (t.is_false()) res = l_False;
    else assert(0);
    return true;
}
...
inline bool AigUtil::aux_depth(const AigLit& t, size_t& res) {
    ...
    if (i == depth_node_cache.end()) return false;
    res = i->second;
    return true;
}


inline bool AigUtil::aux_sz(const AigLit& t, std::unordered_map<size_t,size_t>& node_cache, size_t& res) {
    if (!t.is_node()) {
        res = 1;
        return true;
    }
