 ...
 */
#ifndef COREMANAGER_H_10678
#define COREMANAGER_H_10678
#include<vector>
#include"minisat_auxiliary.h"
using SATSPC::LSet;

//the literal definition used to represent moves, implemented using hash consing of lits and bit-vectors

/*
 * A set of literals implemented with reference counting and containing two redundant representations:
 * a factor of literals and a bit-vector of literals, ensuring thus fast access as well as iterations.
 */
class Core {
    public:
        ...
        const_iterator begin() const { return c ? c->lits.begin() : empty_lits.begin(); }
        const_iterator end() const { return  c ? c->lits.end() : empty_lits.end(); }
        Core() : c(NULL) {}
        Core(const Core& o) { if ((c = o.c)) c->inc_ref(); }
    ...
    protected:
        ...
        struct Rec {
            ...
            inline bool insert(Lit l) {
                const size_t li = literal_index(l);
                if (li >= bv.size()) bv.resize(li+1, false);
                if (bv[li]) return false;
                bv[li] = true;
                lits.push_back(l);
                return true;
            }
...
inline Lit get_vlit(SATSPC::Var v, const Core& core) {
    ...
    if (core.has(pl)) return pl;
    if (core.has(nl)) return nl;
    return SATSPC::lit_Undef;
}

inline std::ostream& operator << (std::ostream& o, const Core& core) {
    o << '[';
    for (size_t i = 0; i < core.size(); ++i) o << (i ? " " : "") << core[i];
    return o << ']';
}
#endif /* COREMANAGER_H_10678 */
