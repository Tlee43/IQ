...
inline lbool lbool_neg(lbool lv) {
    if (lv == l_True) return l_False;
    if (lv == l_False) return l_True;
    assert(lv == l_Undef);
    return l_Undef;
}
...
inline void set(Var v, lbool val, vec<lbool>& vals) {
  if (v >= vals.size()) vals.growTo(v+1, l_Undef);
  vals[v] = val;
}
...
inline void set(const std::vector<Var>& vars, const vec<lbool>& src_vals, vec<lbool>& dst_vals) {
    for (Var v : vars) set(v, eval(v, src_vals), dst_vals);
}
...
inline void to_lits(const std::vector<Var>& vars, const vec<lbool>& bv, vec<Lit>& output, bool complete) {
    for (Var v : vars) {
        lbool val =  eval(v, bv);
        if (complete && val == l_Undef) val = l_False;
        if (val==l_True) output.push(mkLit(v));
        else if (val==l_False) output.push(~mkLit(v));
  }
}
...
inline std::ostream& operator << (std::ostream& o, const SATSPC::LSet& core) {
    o << '[';
    for (int i = 0; i < core.size(); ++i) o << (i ? " " : "") << core[i];
    return o << ']';
}
...
namespace std {
    template<>
    struct hash<pair<SATSPC::Var, SATSPC::Var> > {
        size_t operator () (const pair<Var,Var> &p) const { return p.first ^ p.second; }
    };
};
