...
AigLit AigUtil::mk_xor(const AigLit& a, const AigLit& b) {
    return factory.mk_and(factory.mk_or(a, b),factory.mk_or(factory.neg(a), factory.neg(b)));
}

AigLit AigUtil::and_lin_(std::vector<AigLit>& xs, bool rmdupl) {
    if (rmdupl) rm_dupls(xs);
    auto retv = factory.mk_true();
    for (AigLit& l : xs) retv = factory.mk_and(retv, l);
    return retv;
}
AigLit AigUtil::or_lin_(std::vector<AigLit>& xs, bool rmdupl) {
    if (rmdupl) rm_dupls(xs);
    auto retv = factory.mk_false();
    for (AigLit& l : xs) retv = factory.mk_or(retv, l);
    return retv;
}

AigLit AigUtil::and_(std::vector<AigLit>& xs, bool rmdupls) {
    if (xs.empty()) return factory.mk_true();
    if (rmdupls) rm_dupls(xs);
    vector<AigLit> a2;
...
//    for(AigLit l:xs) factory.print(std::cerr, l)<<std::endl;
    ...
    if (xs.empty()) return factory.mk_false();
    vector<AigLit> a2;
    if (rmdupls) rm_dupls(xs);
    auto p1 = &xs;
    auto p2 = &a2;
...
AigLit AigUtil::convert(const CNF& cnf) {
    vector<AigLit> a1;
    for (const LitSet& ls : cnf) a1.push_back(convert_clause(ls));
...
AigLit AigUtil::convert_clause(const LitSet& ls) {
    ...
    for (size_t i = 0; i < ls.size(); ++i) a1[i] = convert(ls[i]);
    return or_(a1, false);
}
...
lbool AigUtil::eval(AigLit fla, const Core& assignment) {
    ...
    while (!todo.empty()) {
        ...
        todo.pop_back();
        if (get_eval(t, assignment, node_cache, tmp)) continue;
        assert(t.is_node());
    ...
    if (!get_eval(fla, assignment, node_cache, tmp)) assert(0);
    return tmp;
}
...
size_t AigUtil::size(const AigLit& fla) {
    ...
    while (!todo.empty()) {
        const AigLit t = todo.back();
        todo.pop_back();
        if (aux_sz(t, node_cache, tmp)) continue;
        const auto ptr = t.ptr();
        const auto n = factory.get_node(ptr);
        size_t ares, bres;
    ...
    if (!aux_sz(fla, node_cache, tmp)) assert(0);
    return tmp;
}
